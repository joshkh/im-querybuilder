...
class UIBlockModel extends CoreModel
  ...
  #
  # # Accepts an array of path ids and adds them to the
  # # query's select
  # addPathsToQuery: (paths) ->
  #   console.log "Adding paths to query"
  #   _.each @get('paths'), (path) =>
  #     if not @get('query').hasView path
  #       @get('query').addToSelect path.id
  #
  #   # @get('query').hasView "Gene.primaryIdentifier"
  #
  # addPaths: (strpaths = []) ->
  ...
  #       @attach impath
  #     @addPathsToQuery @get('paths')
  #     @trigger 'change:paths', @
  #
  #
  # attach: (item) ->
  #   x = new PathModel item
  #   @get('paths').push x
  #
  # addSummaryFields: ->
